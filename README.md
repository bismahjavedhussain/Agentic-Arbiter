...
| **Only Ashburn has a calibration of its own** | A *field* is one call; a *calibration* needs a forecast leg and its elapsed outcome. Every site's hours, weather and geometry are its own, but the measured level offset and coverage record are Ashburn's. Each borrowing site declares it, and `audit.py` fails if one does not. |
