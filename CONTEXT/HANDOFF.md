# HANDOFF — FortyGuard Hackathon'26 · AGENTIC-ARBITER
...
>
> 1. **START HERE:** `cd AGENTIC-ARBITER/src && python run_all.py` → **25 steps, ZERO API calls,
...
5. **Every claim in `AGENTIC-ARBITER/PLAN.md` carries a real citation with a link**, verified by
...
10. **Document as you go** — `AGENTIC-ARBITER/PLAN.md` is the live design record; root
...
**`AGENTIC-ARBITER`** — an agent that decides, hour by hour, whether a data centre can switch its
...
| | |
|---|---|
| **The agent loop, 7 stages** | `AGENTIC-ARBITER/src/agent.py`. **120,960 scenarios** across 8 swept axes. Zero API calls, ~37 s |
...
1. **`cd AGENTIC-ARBITER/src && python run_all.py`** → must end `REBUILD COMPLETE`. Quote nothing otherwise.
...
8. **The calibration collector is a Windows scheduled task**, `AGENTIC-ARBITER n26 calibration`, two
...
```bash
git checkout submission-safe-2026-08-27 -- AGENTIC-ARBITER/
```
...
🟢 **SCHEDULED: `AGENTIC-ARBITER rebuild calibration`, ONE-TIME, 2026-08-28 16:00 PKT.**
...
rollback    on failure: git checkout -- AGENTIC-ARBITER/{demo,data}, then re-audit and say so
```
...
2. **THE COLLECTORS ARE LIVE AND THE MACHINE MUST BE ON 13:25–15:35 PKT DAILY.** Four tasks:
   `AGENTIC-ARBITER n26 calibration` (13:30, 15:30, Ashburn) · `FG-N26-Chicago-Offset`
...
**Run it:** `cd AGENTIC-ARBITER/demo && python -m http.server 8000` → `http://localhost:8000`.
...
```bash
cd AGENTIC-ARBITER/src && python run_all.py      # 25 steps, ~360 s, zero API calls, non-zero on failure
cd AGENTIC-ARBITER/src && python build_sites.py # just the per-site chain (agent..report) for each site
cd AGENTIC-ARBITER/src && python report.py      # just the PDFs, verified by being read back
cd AGENTIC-ARBITER/src && python ticker.py       # prints the whole tape -- READ IT, see #76
...
1. `cd AGENTIC-ARBITER/src && python run_all.py` → **25 steps, 169 checks, ZERO API calls.**
...
7. 🟢 **THERE IS A TAGGED, SUBMITTABLE FALLBACK: `submission-safe-2026-08-27` (`45aa05c`)** — green
   ...
   `git checkout submission-safe-2026-08-27 -- AGENTIC-ARBITER/`. **So work is no longer
...
| `RECIRCULATION-PHYSICS.md` | why the plume physics is in the product when the rise is 0.36 °C — the question a judge will ask |
...
| `AGENTIC-ARBITER/PLAN.md` §12 | the citation register — every load-bearing claim with a source |
...
- **`FORTYGUARD_API_KEY`** in `d:\FGHackathon\.env` **and** `d:\FGHackathon\AGENTIC-ARBITER\.env`,
...
3. **2026-08-18: the key was pasted into `AGENTIC-ARBITER/.env.example`** — the one dotfile
...
**New scripts in `AGENTIC-ARBITER/src/`, in pipeline order:**
...
**Modified in `AGENTIC-ARBITER/src/`:**
...
**Modified in `AGENTIC-ARBITER/demo/`:**
...
## 13.1 `AGENTIC-ARBITER/src/` — 30 modules, **ALL COMMITTED**
...
## 13.2 `AGENTIC-ARBITER/demo/` — the interface, ~41 MB
...
**`HANDOFF.md`** (this file) · `AGENTIC-ARBITER/PLAN.md` (**current** — §8n/§8o/§8p/§8q/§1a/§12.8a
...
| **`d57b3b7`** | **Session D** + the whole `AGENTIC-ARBITER/` tree, 186 files, 194 MB — it had been untracked |
