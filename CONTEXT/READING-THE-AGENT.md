...
## 2.1 The bar across the top — the "bezel"

A thin bar stays at the top of the page whatever you scroll to. **Bezel** is the word for the rim
around an instrument's dial — the frame that carries the labels rather than the reading. It holds
three things and nothing else:

```
  Agentic·Arbiter   ① PICK A SITE   ② CONFIGURE THE PLANT   ③ READ THE DECISION       ☾
  └ the wordmark    └ which of the three stages you are on                        └ light /
                       (a sliding pill marks where you are, a finished step stays        dark
                        clickable so you can go back, and a later one does not)
```

The reason it exists: the page used to open with the title and five paragraphs of explanation, and
that block stayed at the top of *every* stage — so once you had read it and run the agent, you had to
scroll back up through it to find out which site you were looking at. Identity and status belong in a
frame that is always there. The explanation now appears once, on the first screen, and steps aside
once you move past it.

## 2.2 The three modes, and the line that says which one you are in

The last line of the opening block is the whole answer to "where did these numbers come from". It is
a sentence rather than a control, and deliberately: which mode you are in is decided by how the page
is being served, so there is nothing on the page that could switch it.

> An earlier draft put this in the bezel as a round lamp with a panel behind it, and this
> section used to describe that. It was taken out on 2026-08-27, because moving the live
> controls away from the results card changed how the page reads. The statement now lives in
> the opening block, and the live control itself stays exactly where it always was: on the
> card at the results stage.

- **REPLAY** (grey lamp) — every number comes from saved responses. No API key, no internet, no
  money. Fully reproducible: the same request to FortyGuard returns byte-for-byte identical data
  (17,862 of 17,862 tiles, measured), which is why saved answers are as good as live ones for showing
  the method. What live adds is not accuracy — it is **recency**: hours that have not happened yet.
- **LIVE · DRY RUN** (amber lamp) — a local agent is attached and is deliberately refusing to spend.
  Ask it for the next hours and it tells you exactly what it *would* fetch and for how many credits,
  and calls nothing. This is the safe default, and it is a *different* state from REPLAY: something
  is there, and it is declining.
- **LIVE** (green lamp) — a local agent is attached, holds a key, and is permitted to spend. The
  card at the results stage can now ask FortyGuard about the next hours and decide them.

**Why a "local agent" and not just a button.** FortyGuard authenticates with a secret key. Anything
a web page can read, every visitor of that page can read — so the key cannot live in the page, and
there is no clever way around that. It lives in a small program on *your* computer
(`src/serve_live.py`), which reads the key, calls FortyGuard, and hands the page back only numbers.

## 2.3 Light and dark

The moon/sun button at the right of the bezel switches the palette, and remembers. If you have never
pressed it, the page follows your operating system's setting. Both palettes are measured rather than
eyeballed: `python testing/verify_palette.py` reads the colour values straight out of the page and
checks every text and graphic pair against the accessibility floors, in both modes. The two *data*
colours — the blue and the orange that mean "agent" and "incumbent" — are identical in both, because
those two were validated as a pair for colour-blind readers and a theme is allowed to change the
paper and the ink but not the marks.
