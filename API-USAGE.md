...
| **Paid calls made** | **46** |
| **Credits spent** | **194,120** |
| **Share of the plan used** | **9.71 %** |
| Credits remaining | **1,805,880** |
...
**All 46 paid calls on this plan were `/v1/heatmap`.** That is not an assumption: 194,120 ÷ 4,220 =
**46 exactly**, with no remainder, and a single `env_params` call at 2,900 would have made the
division impossible.

---

## 3. The 46 calls, itemised
...
| Returned `completed` with **zero** features, individually attributed | **27** | 113,940 |
| Not individually attributable — a gap between two readings | **12** | 50,640 |

So **58.7 %** of spend is *proven* to have bought nothing, and the ceiling — if every unattributable
call also failed — is **84.8 %**. The vendor record makes the ceiling far likelier than the floor:
