...
| **Paid calls made** | **212** — 205 `heatmap` + 7 `env_params` |
| **Credits spent** | **885,400** |
| **Share of the plan used** | **44.27 %** |
| Credits remaining | **1,114,600** |
...
one: **205 × 4,220 + 7 × 2,900 = 885,400**, matching the lowest meter reading ever recorded to the
...
## 3. The 212 calls, itemised

**112 calls** saved a before/after meter pair and so are individually attributable; the remaining
**96** are visible only as gaps between readings and are counted, not named. Five of the attributable
...
⚠ The 96-call gap figure is **derived at the heatmap price** and so is approximate to ±1: the gaps
...
| Not individually attributable — a gap between two readings | **109** | 459,980 |

Those three rows sum to 212 and to 885,400, which is the check that keeps this table honest.

So **30.0 %** of spend is *proven* to have bought nothing, and the ceiling — if every unattributable
call also failed — is **82.0 %**. The vendor record makes the ceiling far likelier than the floor:
...
records **12** failed attempts across **5** days; at least one of those cost **0** credits, because
