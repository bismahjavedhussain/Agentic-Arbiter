# AGENTIC-ARBITER — the plan, and the road that got here
...
**FortyGuard's forecast is precisely the missing input.** AGENTIC-ARBITER turns it into an hour-by-hour
...
| **AGENTIC-ARBITER** | **a surface** computed at runtime from the ensemble, the switch budget and the coverage record. **Not stored anywhere; nobody can state it in advance** |
...
> **AGENTIC-ARBITER applies to facilities whose cooling equipment draws air within a few metres of grade
...
```bash
cd AGENTIC-ARBITER/src && python run_all.py     # 65 s, zero API calls, exits non-zero on any failure
```
