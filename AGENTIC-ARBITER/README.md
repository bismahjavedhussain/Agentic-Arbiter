...
AGENTIC-ARBITER is the agent that turns it into switching decisions, bounds its own uncertainty, and then
...
| **AGENTIC-ARBITER** | **a surface** computed at runtime from ensemble spread, switch budget and its own coverage record. **Not stored anywhere. Nobody can state it in advance.** |
