...
#
# The scope is the SHIPPED tree: AGENTIC-ARBITER/src/*.py and AGENTIC-ARBITER/demo/*.{js,html,json}.
RETIRED_CONSTANTS = [
    ("wetbulb_margin_c",
...
#   AGENTIC-ARBITER/demo/README.md carried "costs hours and buys safety" against a registry holding
...
def check_front_door_figures():
    ...
    # HOW MANY OFFERABLE SITES HAVE ONE IMAGERY SOURCE, TWO, OR NONE. Counted from the manifest the
    # page itself reads, so the README's sentence about the two-source cross-check is re-derived on
    # every run instead of being three numbers typed once. `sources` is a dict of {vendor: filename},
    # and a site with no screening frame has no entry at all -- three states, and the README states
    # all three, because lumping "one frame" together with "no frame" would make the sentence wrong
    # for the two sites that have neither.
    _sj = jload(os.path.join(DEMO, "sites.json"))
    _off = [x for x in _sj["sites"] if x.get("offerable")]
    _nsrc = [len(((x.get("imagery") or {}).get("sources") or {})) for x in _off]
    IMGN = len(_off)
    IMG2 = sum(1 for n in _nsrc if n >= 2)
    IMG1 = sum(1 for n in _nsrc if n == 1)
    IMG0 = sum(1 for n in _nsrc if n == 0)
    ...
    want = [
        ...
        # ---- IMAGERY SOURCING, registered 2026-08-27 -------------------------------------------
        # The single-source caveat ("no second source, so the two-source cross-check is NOT met
        # here") was on the site panel, where it was true of 245 of 250 sites -- a property of the
        # project rather than news about the site a reader had just opened -- and it moved into the
        # README's honest-limits table at the user's direction. Moving a disclosure into prose is
        # exactly when the prose needs a check: three counts stated in English and re-read by
        # nothing is gotcha #67, and these three will drift the moment the national tier gains a
        # second frame anywhere. Counted from sites.json, so the sentence cannot go stale silently.
        ("imagery: two-source sites",  "**%d of %d offerable sites**" % (IMG2, IMGN)),
        ("imagery: single-source",     "**%d carry exactly one**" % IMG1),
        ("imagery: no frame at all",   "**%d have no screening frame at all**" % IMG0),
...
def main():
    print("=" * 78)
    print("AUDIT -- AGENTIC-ARBITER, whole tree")
    print("=" * 78)
    check_dead_code()
    check_nan_writers()
    check_css_comments()
